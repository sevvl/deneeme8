    ...
    if analysis:
        ...
        if analysis.disease_detected and analysis.disease_detected != "Unknown":
            ...
            if analysis.symptoms:
                st.subheader("Belirtiler")
                st.markdown(analysis.symptoms)
            if analysis.possible_causes:
                st.subheader("Olası Nedenler")
                st.markdown(analysis.possible_causes)
            if analysis.recommended_actions:
                st.subheader("Önerilen Eylemler / Tedaviler")
                st.markdown(analysis.recommended_actions)
            if analysis.prevention_strategies:
                st.subheader("Önleme Stratejileri")
                st.markdown(analysis.prevention_strategies)

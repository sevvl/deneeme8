...
class DatabaseService:
    ...
    def add_analysis(self, analysis: Analysis) -> Optional[int]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO analyses (user_id, image_path, disease_detected, confidence_score, analysis_date, gemini_response, explanation, symptoms, possible_causes, recommended_actions, prevention_strategies) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (analysis.user_id, analysis.image_path, analysis.disease_detected, analysis.confidence_score, analysis.analysis_date, analysis.gemini_response, analysis.explanation, analysis.symptoms, analysis.possible_causes, analysis.recommended_actions, analysis.prevention_strategies)
        )
        conn.commit()
        return cursor.lastrowid
    ...
    def get_analysis_by_id(self, analysis_id: int) -> Optional[Analysis]:
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, user_id, image_path, disease_detected, confidence_score, analysis_date, gemini_response, explanation, symptoms, possible_causes, recommended_actions, prevention_strategies FROM analyses WHERE id = ?", (analysis_id,))
        row = cursor.fetchone()
        if row:
    ...
    def get_analyses_by_user_id(self, user_id: int) -> List[Analysis]:
        conn = self._get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, user_id, image_path, disease_detected, confidence_score, analysis_date, gemini_response, explanation, symptoms, possible_causes, recommended_actions, prevention_strategies FROM analyses WHERE user_id = ? ORDER BY analysis_date DESC", (user_id,))

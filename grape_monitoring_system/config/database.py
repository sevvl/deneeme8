    ...
    try:
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS analyses (
                ...
                explanation TEXT, # New field
                symptoms TEXT, # New field
                possible_causes TEXT,
                recommended_actions TEXT, # New field
                prevention_strategies TEXT, # New field
                FOREIGN KEY (user_id) REFERENCES users (id)
            )
        """)
        # Check for existing columns and add if not present (for backward compatibility and schema evolution)
        ...
        for col, col_type in columns_to_add.items():
            if col not in existing_columns:
                ...
                print(f"Database: Added '{col}' to analyses table.")

        # Columns to remove (if they exist) - SQLite doesn't directly support DROP COLUMN, requires more complex migration
        # For simplicity, we'll assume these columns will not be actively used if they exist, or manual migration
        # is handled if a fresh DB is not an option.
